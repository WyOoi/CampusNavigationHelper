    ...
    def display_menu(self):
        ...
        def display_simulate_route(*args, **kwargs):
            ...
            def plot_route():
                ...
                if start and end:
                    google_maps_url = f"https://www.google.com/maps/dir/?api=1&origin={start}&destination={end}&travelmode=walking"
                    webbrowser.open(google_maps_url)
                    tk.Label(content_frame, text="Route opened in Google Maps!", fg="green").pack(pady=10)
                else:
                    messagebox.showerror("Error", "Please enter both a starting location and a destination.")

            tk.Button(content_frame, text="Plot Route on Google Maps", command=plot_route).pack(pady=20)
            tk.Label(content_frame, text="Enter the starting and destination locations.", fg="gray").pack(pady=10)
